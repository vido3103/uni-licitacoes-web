...
export default function CompanyOnboarding(){
  ...
  async function confirm(){if(!supabase||!lookup)return;setSaving(true);setError("");try{const {data,error}=await supabase.functions.invoke("company-onboarding-start",{body:{cnpj:lookup.cnpj,legal_name:lookup.legal_name||lookup.trade_name||"Empresa",trade_name:lookup.trade_name,city:lookup.city,state:lookup.state,cnaes:lookup.cnaes,source_data:{provider:lookup.source,queried_at:new Date().toISOString(),registration_status:lookup.registration_status,opened_at:lookup.opened_at,address:{street:lookup.street,number:lookup.number,complement:lookup.complement,district:lookup.district,postal_code:lookup.postal_code}}}});if(error)throw error;if(!data?.ok)throw new Error(data?.detail||data?.error||"Não foi possível criar o cadastro.");setDone(data as {client_id:string;login:string;status:string})}catch(e){setError(errorText(e))}finally{setSaving(false)}}
