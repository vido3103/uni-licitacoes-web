...
import { errorMessage } from "@/modules/radar/radar-utils";
...
export default function ApprovedClientOnboarding({ onNavigate }: { onNavigate: (module: string) => void }) {
  ...
  async function consult() {
    ...
    try {
      ...
      if (error) throw error;
      if (!data?.ok) throw new Error(data?.error || "Não foi possível consultar o CNPJ.");
  ...
  async function confirm() {
    ...
    try {
      ...
      if (error) throw error;
      if (!data?.ok) throw new Error(data?.detail || "Não foi possível atualizar o cadastro.");
      setDone(true);
  ...
  return <div className="mx-auto w-full max-w-5xl p-4 sm:p-6 xl:p-8">
    ...
    {done && <section className="mt-5 rounded-2xl border border-blue-200 bg-blue-50 p-5"><h2 className="font-bold text-blue-900">Próximas etapas</h2><p className="mt-2 text-sm text-blue-900">Anexe os documentos da empresa e acompanhe a página Habilitação até a validação final do Owner.</p><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => onNavigate("Documentos")} className="rounded-xl bg-blue-700 px-4 py-2.5 text-sm font-bold text-white">Ir para Documentos</button><button onClick={() => onNavigate("SICAF")} className="rounded-xl border border-blue-300 bg-white px-4 py-2.5 text-sm font-bold text-blue-700">Ir para Habilitação</button></div></section>}
  </div>;
}
