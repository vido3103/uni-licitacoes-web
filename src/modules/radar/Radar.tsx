...
export default function Radar() {
  ...
  async function loadDocuments(opportunity: Opportunity) {
    if (!supabase || !data?.client?.id || !opportunity.id) return;
    setDocsLoading(true);
    setUploadMessage("");
  ...
  async function uploadFiles(files: FileList | null) {
    if (!files?.length || !supabase || !data?.client?.id || !selected?.id) return;
    setUploading(true);
    setUploadMessage("");
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        const path = `${data.client.id}/${String(selected.id)}/${Date.now()}-${safeName(file.name)}`;
          ...
          <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
              ...
              <div className="rounded-xl border border-slate-200 p-4">
                ...
                <button type="button" disabled={documents.length === 0} className="mt-5 w-full rounded-xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-slate-300">{documents.length > 0 ? "Iniciar / complementar Análise Detalhada" : "Anexe ao menos um documento para continuar"}</button>
              </div>
            </div>
          </section>
        )}
