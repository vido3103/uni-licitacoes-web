...
const fmtDateTime=(v:string|null)=>v?new Intl.DateTimeFormat("pt-BR",{dateStyle:"short",timeStyle:"short"}).format(new Date(v)):"—";
const fmtDateOnly=(v:string|null)=>v?new Intl.DateTimeFormat("pt-BR").format(new Date(`${v}T12:00:00`)):"—";
...
export default function SicafHabilitacao(){
 ...
 async function load(preserveMessage=false){
  if(!supabase)return;setLoading(true);if(!preserveMessage)setMsg("");
  try{
   ...
   if(identity.error)throw identity.error;if(docTypes.error)throw docTypes.error;if(documents.error)throw documents.error;if(reviewRow.error)throw reviewRow.error;
   setIsOwner(Boolean(identity.data&&typeof identity.data==="object"&&(identity.data as{platform_role?:string}).platform_role==="platform_owner"));
 ...
 async function upload(file:File|null){
  ...
  try{
   ...
   previousCurrent=current.map(d=>d.id);path=`${clientId}/${target.id}/${Date.now()}-${safe(file.name)}`;
   ...
   if(previousCurrent.length){const{error}=await supabase.from("client_documents").update({is_current:false}).in("id",previousCurrent);if(error)throw error}
   ...
   if(error){if(previousCurrent.length)await supabase.from("client_documents").update({is_current:true}).in("id",previousCurrent);await supabase.storage.from("client-documents").remove([path]);throw error}
   setMsg(review?.habilitado?"Documento anexado. Como o conjunto documental mudou, a habilitação voltou automaticamente para análise.":"Documento anexado com sucesso.");
   setForm(v=>({...v,expiry_date:"",issuing_body:"",observations:""}));await load(true);
  }catch(e){if(path)await supabase.storage.from("client-documents").remove([path]).catch(()=>undefined);setMsg(`Falha no envio: ${e instanceof Error?e.message:String(e)}`)}finally{setBusy(false)}
 }
 ...
 return <div className="mx-auto max-w-[1300px] p-4 sm:p-6 xl:p-8">
  ...
  <section className="mt-5 rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-bold">Situação do cadastro</h2><p className={`mt-1 text-sm font-semibold ${review?.habilitado?"text-emerald-700":"text-amber-700"}`}>{review?.habilitado?"Cliente habilitado":"Em análise"}</p>{review?.validated_at&&<p className="mt-1 text-xs text-slate-500">Validado em {fmtDateTime(review.validated_at)}</p>}</div>{isOwner&&<label className="flex cursor-pointer items-center gap-3 rounded-xl border px-4 py-3"><input type="checkbox" checked={Boolean(review?.habilitado)} disabled={saving} onChange={e=>void setHabilitado(e.target.checked)} className="h-5 w-5"/><span className="text-sm font-bold">Cliente habilitado</span></label>}</div>{isOwner&&<textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Observações do Owner sobre o cadastro..." className="mt-4 min-h-20 w-full rounded-xl border p-3 text-sm"/>}</section>
  <section className="mt-5 rounded-2xl border bg-white p-5 shadow-sm"><h2 className="font-bold">Anexar documentos</h2><p className="mt-1 text-xs text-slate-500">Selecione o tipo, informe os dados disponíveis e anexe o arquivo. Qualquer substituição documental após a aprovação devolve o cadastro para análise do Owner.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><select value={form.typeId} onChange={e=>setForm(v=>({...v,typeId:e.target.value}))} className="rounded-xl border px-3 py-3 text-sm">{types.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select><input type="date" value={form.expiry_date} onChange={e=>setForm(v=>({...v,expiry_date:e.target.value}))} className="rounded-xl border px-3 py-3 text-sm"/><input placeholder="Órgão emissor" value={form.issuing_body} onChange={e=>setForm(v=>({...v,issuing_body:e.target.value}))} className="rounded-xl border px-3 py-3 text-sm"/><input placeholder="Observações" value={form.observations} onChange={e=>setForm(v=>({...v,observations:e.target.value}))} className="rounded-xl border px-3 py-3 text-sm"/></div><label className="mt-3 flex cursor-pointer justify-center rounded-xl border-2 border-dashed p-6 text-sm font-semibold"><span>{busy?"Enviando...":"Selecionar arquivo para enviar"}</span><input type="file" accept=".pdf,.zip,.rar,.7z,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png" disabled={busy} onChange={e=>{const f=e.currentTarget.files?.[0]??null;e.currentTarget.value="";void upload(f)}} className="hidden"/></label></section>
  <section className="mt-5 rounded-2xl border bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="font-bold">Arquivos anexados</h2><p className="mt-1 text-xs text-slate-500">{currentDocs.length} documento(s) atual(is)</p></div><button onClick={()=>void load()} className="rounded-lg border px-3 py-2 text-xs font-semibold">Atualizar</button></div><div className="mt-4 space-y-2">{currentDocs.length?currentDocs.map(d=>{const t=types.find(x=>x.id===d.document_type_id);return <div key={d.id} className="flex items-center justify-between gap-3 rounded-xl border p-3"><div className="min-w-0"><p className="truncate text-sm font-semibold">{d.original_filename}</p><p className="mt-1 text-xs text-slate-500">{t?.name??"Documento"} · versão {d.version} · validade {fmtDateOnly(d.expiry_date)}</p></div><button onClick={()=>void openDoc(d)} className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700">Abrir</button></div>}):<p className="text-sm text-slate-500">Nenhum documento anexado.</p>}</div></section>
