...
export default function Relatorios() {
  ...
  async function load() {
    ...
    try {
      ...
      const [g, s, q] = await Promise.all([
        supabase!.from("gate_economic_results").select("id,opportunity_id,cfp_item_id,quantity,final_unit_cost,suggested_unit_price,estimated_unit_value,viability_status,target_markup_percent,updated_at").eq("client_id", d.client.id),
        supabase!.from("dispute_strategies").select("id,opportunity_id,cfp_item_id,opening_unit_price,minimum_unit_price,status,updated_at").eq("client_id", d.client.id),
        supabase!.from("opportunity_ai_analysis_queue").select("id,opportunity_id,status,prompt_master_version,queued_at,completed_at,error_detail").eq("client_id", d.client.id).order("queued_at", { ascending: false }),
      ]);
      if (g.error || s.error || q.error) throw g.error || s.error || q.error;
      setGates((g.data ?? []) as Gate[]);
      setStrategies((s.data ?? []) as Strategy[]);
      setQueue((q.data ?? []) as Queue[]);
  ...
  const gateRows = opportunityId ? gates.filter(g => g.opportunity_id === opportunityId) : gates;
  const disputeRows = opportunityId ? strategies.filter(s => s.opportunity_id === opportunityId) : strategies;
  const queueRows = opportunityId ? queue.filter(q => q.opportunity_id === opportunityId) : queue;
  const totals = useMemo(() => gateRows.reduce((a, g) => {
    const qty = Number(g.quantity) > 0 ? Number(g.quantity) : 1;
    a.cost += Number(g.final_unit_cost) * qty;
    a.revenue += Number(g.suggested_unit_price) * qty;
    a.quantity += qty;
    if (g.viability_status === "viable") a.viable++;
    if (g.viability_status === "not_viable") a.notViable++;
    return a;
  }, { cost: 0, revenue: 0, quantity: 0, viable: 0, notViable: 0 }), [gateRows]);

  function exportCsv() {
    const header = ["Processo", "Órgão", "Objeto", "Prazo", "Status"];
    const rows = visibleOpps.map(o => [text(o.process_number, ""), text(o.buyer_name, ""), text(o.object_text ?? o.title, ""), text(o.proposal_deadline, ""), text(o.match_status, "")]);
    const csv = [header, ...rows].map(r => r.map(v => `"${String(v).replaceAll('"', '""')}"`).join(";")).join("\n");
    const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob); const a = document.createElement("a");
    a.href = url; a.download = `uni-relatorio-${new Date().toISOString().slice(0, 10)}.csv`; a.click(); URL.revokeObjectURL(url);
  }
  ...
  return <div className="mx-auto max-w-[1500px] p-4 sm:p-6 xl:p-8 print:p-0">
    ...
    <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm print:border-0 print:shadow-none">
      ...
      {gateRows.length > 0 && <div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-slate-100 p-3 text-xs"><span className="text-slate-500">Quantidade consolidada</span><p className="mt-1 font-bold">{totals.quantity}</p></div><div className="rounded-xl border border-slate-100 p-3 text-xs"><span className="text-slate-500">Custo consolidado</span><p className="mt-1 font-bold">{money(totals.cost)}</p></div><div className="rounded-xl border border-slate-100 p-3 text-xs"><span className="text-slate-500">Receita sugerida</span><p className="mt-1 font-bold">{money(totals.revenue)}</p></div></div>}
