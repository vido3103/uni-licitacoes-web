...
const strongPassword = (v: string) =>
  ...
  /\d/.test(v) &&
  /[^A-Za-z0-9]/.test(v);
async function invokeDetail(err: unknown) {
...
export default function AuthGate({ children }: { children: ReactNode }) {
  const [ready, setReady] = useState(true),
    ...
    [mode, setMode] = useState<Mode>("login"),
  ...
  async function signIn(e: FormEvent) {
    ...
    try {
      ...
      if (invokeError || !data?.access_token || !data?.refresh_token) {
        setError(
          "Não foi possível entrar. Confira usuário e senha ou confirme se seu acesso já foi aprovado.",
        );
        return;
      }
  ...
  async function register(e: FormEvent) {
    ...
    try {
      ...
      if (!normalized.startsWith("UNI-"))
        throw new Error("O usuário deve começar com UNI-. Ex.: UNI-LUVI");
      if (!strongPassword(password))
        throw new Error(
          "Use uma senha com pelo menos 12 caracteres, incluindo maiúscula, minúscula, número e símbolo.",
        );
      const { data, error: invokeError } = await supabase.functions.invoke(
      ...
      if (invokeError)
        throw new Error(data?.detail || (await invokeDetail(invokeError)));
      if (!data?.ok)
        throw new Error(
          data?.detail ||
            data?.error ||
  ...
  return (
    <main className="min-h-screen bg-[#071525] text-white">
      <div className="mx-auto grid min-h-screen max-w-[1320px] items-center gap-12 px-6 py-10 lg:grid-cols-[1.05fr_.75fr] lg:px-10">
        <section className="hidden lg:block">
          ...
          <h1 className="mt-4 max-w-2xl text-5xl font-black leading-[1.06] tracking-[-.04em]">
            Da oportunidade à decisão, em um único ambiente.
          </h1>
          <p className="mt-6 max-w-xl text-base leading-7 text-slate-400">
            Acompanhe oportunidades aprovadas, participações e resultados com
          ...
          <div className="mt-10 grid max-w-xl grid-cols-3 gap-3">
            {[
              ...
              >
                <p className="text-[10px] font-bold text-blue-400">{n}</p>
          ...
          <div className="rounded-[28px] border border-white/10 bg-white p-7 text-slate-900 shadow-2xl shadow-black/30 sm:p-9">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black tracking-tight">
                {mode === "login" ? "Acesse sua conta" : "Cadastre sua empresa"}
              </h2>
            ...
            <div className="my-6 grid grid-cols-2 rounded-xl bg-slate-100 p-1 text-xs font-bold">
              <button
                type="button"
                onClick={() => {
                  setMode("login");
                ...
                className={`rounded-lg px-3 py-2.5 ${mode === "login" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500"}`}
              >
                Entrar
              </button>
              <button
                ...
                className={`rounded-lg px-3 py-2.5 ${mode === "register" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500"}`}
              >
                Nova empresa
              </button>
            </div>
            <form
              onSubmit={mode === "login" ? signIn : register}
              className="space-y-4"
            >
              {mode === "register" && (
                <>
              ...
              <label className="block text-xs font-bold text-slate-700">
                ...
                <input
                  ...
                  placeholder={mode === "register" ? "UNI-LUVI" : "UNI-EMPRESA"}
                  className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm uppercase outline-none focus:border-blue-500 focus:bg-white"
                />
              </label>
              <label className="block text-xs font-bold text-slate-700">
                Senha
              ...
              {error && (
                <p className="rounded-xl bg-rose-50 p-3 text-xs font-medium leading-5 text-rose-700">
                  {error}
                </p>
              )}
              {success && (
                <p className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs font-medium leading-5 text-emerald-800">
                  {success}
                </p>
              )}
              <button
                type="submit"
                disabled={loading}
                className="w-full rounded-xl bg-blue-600 px-4 py-3.5 text-sm font-black text-white shadow-lg shadow-blue-200 transition hover:bg-blue-700 disabled:opacity-60"
              >
                {loading
                  ? "Processando..."
                  : mode === "login"
