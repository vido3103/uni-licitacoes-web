...
export async function resolveCurrentClientId():Promise<string|null>{
  ...
  if(selected){
    const{data,error}=await supabase.from("clients").select("id").eq("id",selected).maybeSingle();
    if(!error&&data?.id)return String(data.id);
    sessionStorage.removeItem("uni-owner-client-id");
  }

  const{data:membership,error}=await supabase.from("client_members").select("client_id").eq("user_id",auth.user.id).limit(1).maybeSingle();
  if(error)throw error;
  return membership?.client_id?String(membership.client_id):null;
}
