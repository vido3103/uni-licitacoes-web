...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(!token)return json({error:"unauthorized",detail:"Sessão ausente. Entre novamente no UNI."},401);
  const{data:userData,error:userError}=await db.auth.getUser(token);if(userError||!userData.user)return json({error:"unauthorized",detail:"Sessão inválida ou expirada. Entre novamente no UNI."},401);
  ...
  if(!clientId||!opportunityId||!capabilityId)return json({error:"missing_context",detail:"Cliente, edital e capacidade são obrigatórios para validar as exigências."},400);
  ...
  if(!member&&!owner)return json({error:"forbidden",detail:"Seu usuário não possui acesso a este cliente."},403);if(!cap)return json({error:"capability_not_in_client",detail:"A capacidade vinculada ao edital não pertence ao cliente selecionado."},403);
  ...
  if(opRes.error||!opRes.data)return json({error:"opportunity_not_found",detail:"O edital não foi localizado no cadastro operacional."},404);if(docsRes.error)return json({error:"documents_query_failed",detail:docsRes.error.message},500);
  const docs=docsRes.data??[];if(!docs.length)return json({error:"no_documents_available",detail:"Nenhum edital/TR disponível. Sincronize ou anexe os documentos antes da validação."},409);
  ...
  if(!included.length)return json({error:"no_supported_pdf_available",detail:"Os documentos disponíveis não puderam ser lidos para estruturar as exigências.",omitted},409);
  const parsed=await gemini(parts,geminiKey);const raw=Array.isArray(parsed?.requirements)?parsed.requirements:[];
  const seen=new Set<string>();const normalized=raw.map((x:any)=>{const title=clean(x?.title,300),description=clean(x?.description,2000),source=clean(x?.source_locator,500),note=clean(x?.notes,1000),kind=ALLOWED_KINDS.has(String(x?.requirement_kind))?String(x.requirement_kind):"other";return{title,description:description||null,requirement_kind:kind,blocking:Boolean(x?.blocking),source_locator:source||null,notes:`AUTO:gemini-requirements-v2${note?` | ${note}`:""}`}}).filter((x:any)=>{const k=x.title.toLowerCase();if(!x.title||seen.has(k))return false;seen.add(k);return true}).slice(0,100);
  const{data:manual,error:manualErr}=await db.from("opportunity_requirements").select("title").eq("client_id",clientId).eq("opportunity_id",opportunityId).eq("capability_id",capabilityId).not("notes","like","AUTO:gemini-requirements-v%");if(manualErr)return json({error:"requirements_query_failed",detail:manualErr.message},500);const manualTitles=new Set((manual??[]).map((x:any)=>String(x.title).toLowerCase()));
  const{error:deleteErr}=await db.from("opportunity_requirements").delete().eq("client_id",clientId).eq("opportunity_id",opportunityId).eq("capability_id",capabilityId).like("notes","AUTO:gemini-requirements-v%");if(deleteErr)return json({error:"requirements_refresh_failed",detail:deleteErr.message},500);
