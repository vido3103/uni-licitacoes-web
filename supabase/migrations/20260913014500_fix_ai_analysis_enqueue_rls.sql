create or replace function private.enqueue_opportunity_ai_analysis_internal(
...
begin
  ...
  if not exists(
    select 1 from public.opportunity_documents d
...
revoke all on function private.enqueue_opportunity_ai_analysis_internal(uuid,uuid,text) from public, anon, authenticated;
...
set search_path=''
as $function$
  select private.enqueue_opportunity_ai_analysis_internal(p_capability_id,p_opportunity_id,p_prompt_master_version);
$function$;
