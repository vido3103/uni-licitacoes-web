-- Veence: prevent an ordinary repeated enqueue from reopening a completed AI job.
-- Intentional reruns must use a future explicit rerun entrypoint so a duplicate UI/HTTP
-- request cannot silently create another paid inference.
...
begin
  ...
  on conflict(client_id,capability_id,opportunity_id,triage_run_id) do update set
    -- A completed job is intentionally left completed. Only failed/retry_wait jobs are
    -- reopened by the ordinary enqueue path.
    prompt_master_version=case when public.opportunity_ai_analysis_queue.status in ('failed','retry_wait') then excluded.prompt_master_version else public.opportunity_ai_analysis_queue.prompt_master_version end,
    profile_version=case when public.opportunity_ai_analysis_queue.status in ('failed','retry_wait') then excluded.profile_version else public.opportunity_ai_analysis_queue.profile_version end,
    playbook_version=case when public.opportunity_ai_analysis_queue.status in ('failed','retry_wait') then excluded.playbook_version else public.opportunity_ai_analysis_queue.playbook_version end,
    context_snapshot=case when public.opportunity_ai_analysis_queue.status in ('failed','retry_wait') then excluded.context_snapshot else public.opportunity_ai_analysis_queue.context_snapshot end,
    status=case when public.opportunity_ai_analysis_queue.status in ('failed','retry_wait') then 'pending' else public.opportunity_ai_analysis_queue.status end,
    error_detail=case when public.opportunity_ai_analysis_queue.status in ('failed','retry_wait') then null else public.opportunity_ai_analysis_queue.error_detail end,
    next_attempt_at=case when public.opportunity_ai_analysis_queue.status in ('failed','retry_wait') then null else public.opportunity_ai_analysis_queue.next_attempt_at end,
    completed_at=case when public.opportunity_ai_analysis_queue.status in ('failed','retry_wait') then null else public.opportunity_ai_analysis_queue.completed_at end,
    started_at=case when public.opportunity_ai_analysis_queue.status in ('failed','retry_wait') then null else public.opportunity_ai_analysis_queue.started_at end,
    locked_at=case when public.opportunity_ai_analysis_queue.status in ('failed','retry_wait') then null else public.opportunity_ai_analysis_queue.locked_at end,
    locked_by=case when public.opportunity_ai_analysis_queue.status in ('failed','retry_wait') then null else public.opportunity_ai_analysis_queue.locked_by end,
    heartbeat_at=case when public.opportunity_ai_analysis_queue.status in ('failed','retry_wait') then null else public.opportunity_ai_analysis_queue.heartbeat_at end,
    updated_at=case when public.opportunity_ai_analysis_queue.status in ('failed','retry_wait') then now() else public.opportunity_ai_analysis_queue.updated_at end
